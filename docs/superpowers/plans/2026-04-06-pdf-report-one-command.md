# PDF Report One-Command Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** SKILL.mdを書き換えて「レポート出して 7203」の1コマンドでフルPhase分析→PDF出力を完結させる

**Architecture:** 新規TSコードなし。pdf-reportのSKILL.mdをフルワークフローに書き換えるだけ。既存のdata-builder.ts / scores.ts / technical.ts / supply-demand.ts / generate.tsをClaude Codeがワークフロー指示に従って順次呼び出す。

**Tech Stack:** SKILL.md（Claude Codeへのワークフロー指示）、既存TS関数群、ラジ株ナビMCP、JQuants API

**Spec:** `docs/superpowers/specs/2026-04-06-pdf-report-one-command-design.md`
...
**Files:**
- Modify: `src/skills/pdf-report/SKILL.md`

- [ ] **Step 1: 現行SKILL.mdのバックアップ確認**

現行SKILL.mdの内容はgit履歴に残っているので、バックアップ不要。直接書き換える。

- [ ] **Step 2: SKILL.mdを書き換え**

以下の内容でSKILL.mdを全面書き換えする:
...
- [ ] **Step 3: SKILL.mdのチェックリスト内バッククォートを修正**

Step 2で書いた内容のWorkflow Checklistセクション内の` ` `（スペース入りバッククォート）を正しい` ``` `（バッククォート3つ）に修正する。これはマークダウン内マークダウンのエスケープ問題を避けるための措置。

- [ ] **Step 4: 差分確認**

Run: `git diff src/skills/pdf-report/SKILL.md`
確認: フロントマターのdescriptionが更新され、ワークフローがStep 0〜10の構成になっていること

- [ ] **Step 5: コミット**

```bash
git add src/skills/pdf-report/SKILL.md
git commit -m "feat(pdf-report): rewrite SKILL.md for one-command full-analysis PDF generation"
```
...
**Files:**
- Modify: `/Users/okahiroyuki/.claude/projects/-Users-okahiroyuki-kabu-dexter/memory/project_pdf_report_status.md`

- [ ] **Step 1: メモリファイル更新**

プロジェクトメモリの「未実装」セクションから「コメント生成のClaude連携」を削除し、「完了」セクションに追加する。
