...
`data-builder.ts`の`buildAnalysisInput()`を使ってAnalysisInputを構築し、テクニカル/需給データで上書きする。
...
**4c. 需給データ上書き（Standard以上 + データ取得成功の場合）:**
```
...
モンテカルロ上昇確率はデフォルト値（0.5）のまま使用する。

## Step 5: 8軸スコア算出

`scores.ts`の`calculateAllScores(analysisInput)`で8軸スコアを算出する。

- 決算時スコア: Step 4aのAnalysisInputから算出 → `scores`
- 現在株価スコア（Light以上）: Step 4aの2回目のAnalysisInputから算出 → `scoresAtReport`（※命名注意: generateのReportInput上は決算時が`scoresAtReport`、現在が`scores`）
...
Step 1〜5の分析結果（財務データ・テクニカル指標・スコア）を参照し、以下を生成する。

### 6a. 各軸の根拠テキスト（details）
...
## Step 7: ReportInput組み立て → PDF生成

Step 1〜6の結果から`ReportInput`を組み立て、`generate.ts`の`generateReport()`でPDFを出力する。
...
```typescript
{
  ...
  industryAvg: {全軸50},
  details: {Step 6aの8軸テキスト},
  summary: "Step 6bのサマリー",
  risks: [Step 6cのリスク配列],
